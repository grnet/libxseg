...
static void *__get_peer_type_data(struct xseg *xseg, uint32_t serial)
{
    ...
    shared_peer_type_data =
        XPTR_TAKE(xseg->shared->peer_type_data, xseg->segment);

    priv->peer_type_data[serial] =
        XPTR_TAKE(shared_peer_type_data[serial], xseg->segment);
...
static int pointer_ok(unsigned long ptr,
                      unsigned long base, uint64_t size, char *name)
{
    int ret = !(ptr >= base && ptr < base + size);
    if (ret) {
        XSEGLOG("invalid pointer '->%s' [%llx on %llx]!\n",
                (unsigned long long) ptr, (unsigned long long) base, name);
    }
    return ret;
}
...
struct xseg *xseg_join(const char *segtypename,
                       const char *segname,
                       const char *peertypename, void (*wakeup)
                        (uint32_t portno))
{
...
struct xseg_port *xseg_alloc_port(struct xseg *xseg, uint32_t flags,
                                  uint64_t nr_reqs)
{
    struct xq *q;
    struct xobject_h *obj_h;
...
xport xseg_forward(struct xseg * xseg, struct xseg_request * req,
                   xport new_dst, xport portno, uint32_t flags)
{
    if (!xseg || !req) {
        XSEGLOG("Invalid argument");
        return NoPort;
    ...
    if (!__validate_port(xseg, new_dst)) {
        XSEGLOG("Couldn't validate new destination (new_dst %lu)", new_dst);
        return NoPort;
    }
...
int xseg_set_req_data(struct xseg *xseg, struct xseg_request *xreq, void *data)
{
    ...
    if (r == -XHASH_ERESIZE) {
        req_data =
            xhash_resize(req_data, xhash_grow_size_shift(req_data), 0, NULL);
        if (req_data) {
                      ...
                      void **data)
{
    ...
    *data = (void *) val;
    if (r >= 0) {
        ...
        if (r == -XHASH_ERESIZE) {
            req_data =
                xhash_resize(req_data, xhash_shrink_size_shift(req_data), 0,
                             NULL);
            if (req_data) {
                xseg->priv->req_data = req_data;
                r = xhash_delete(req_data, (xhashidx) xreq);
...
struct xseg_port *xseg_bind_port(struct xseg *xseg, uint32_t req, void *sd)
{
    ...
    for (; portno < maxno; portno++) {
        ...
        if (!sd) {
            ...
            sigdesc =
                xseg->priv->peer_type.peer_ops.alloc_signal_desc(xseg,
                                                                 peer_data);
            if (!sigdesc) {
                break;
            }
            ...
            if (r < 0) {
                xseg->priv->peer_type.peer_ops.free_signal_desc(xseg,
                                                                peer_data,
                                                                sigdesc);
                break;
            }
