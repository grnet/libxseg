...
void report_request(struct xseg_request *req)
{
    ...
    fprintf(stderr,
            ...
            (unsigned long) req, req->targetlen,
            (unsigned long long) req->target, target,
            (unsigned long long) req->datalen, (unsigned long long) req->data,
            data, (unsigned long long) req->offset,
            (unsigned long long) req->size, (unsigned long long) req->serviced,
            req->op, req->state, req->flags, (unsigned int) req->src_portno,
            (unsigned int) req->transit_portno, (unsigned int) req->dst_portno,
...
int cmd_read(char *target, uint64_t offset, uint64_t size)
{
    ...
    char *req_target;
    struct xseg_request *req =
        xseg_get_request(xseg, srcport, dstport, X_ALLOC);
    if (!req) {
        fprintf(stderr, "No request\n");
        return -1;
    }
...
int cmd_delete(char *target)
{
    ...
    if (r < 0) {
        fprintf(stderr, "Cannot prepare request! (%lu, %lu)\n",
                ...
                (unsigned long) req->bufferlen - targetlen);
        xseg_put_request(xseg, req, srcport);
        return -1;
    }
...
int cmd_clone(char *src, char *dst, long size)
{
    ...
    int r =
        xseg_prep_request(xseg, req, targetlen,
             ...
             struct xseg_request *req)
{
    ...
    case 1:
        fprintf(logfp,
                ...
                (unsigned int) portno1,
                (unsigned int) portno2, (unsigned int) req->op);
        break;
    case 2:
        fprintf(logfp, "src port: %u, dst port: %u, reqs: %llu\n",
                (unsigned int) portno1,
                (unsigned int) portno2, (unsigned long long) ++reqs);
    }
...
int cmd_rndwrite(long loops, int32_t seed, uint32_t targetlen,
                 uint32_t chunksize, uint64_t size)
{
...
int cmd_rndread(long loops, int32_t seed, uint32_t targetlen,
                uint32_t chunksize, uint64_t size)
{
    struct xseg_request *submitted = NULL, *received;
    ...
    for (;;) {
        ...
        if (nr_submitted % 1000 == 0 && !reported) {
            reported = 1;
            fprintf(stderr,
                    "submitted %ld, received %ld, failed %ld, mismatched %ld\n",
    ...
    for (;;) {
        ...
        if (nr_submitted < loops && nr_flying < concurrent_reqs &&
            ...
            if (op == 0) {
                ...
                submitted->op = X_WRITE;
                mkchunk(req_data, submitted->datalen, req_target,
                        submitted->targetlen, submitted->offset);
            }
        ...
        if (received) {
            ...
            if (nr_received == 0) {
                fprintf(stderr,
                        "latency (time for the first req to complete): %llu usecs\n",
...
static void print_hanlder(char *name, struct xobject_h *obj_h)
{
    ...
    fprintf(stderr,
            "%20s: free: %4llu, allocated: %4llu, allocated space: %7llu (object size: %llu), Lock %s\n",
            name, (unsigned long long) obj_h->nr_free,
...
static void print_heap(struct xseg *xseg)
{
    ...
    fprintf(stderr, "(%llu / %llu), %s\n",
            ...
            (unsigned long long) xseg->config.heap_size, ls);
}
    ...
    for (; nr--;) {
        ...
        if (req) {
            ...
            if (fail == 1) {
                ...
                if (req->op == X_READ) {
                    mkchunk(req_data, req->datalen, req_target, req->targetlen,
                            req->offset);
                } else if (req->op == X_WRITE) {
                    memcpy(buf, req_data,
                           (sizeof(*buf) >
                            req->datalen) ? req->datalen : sizeof(*buf));
...
int cmd_put_replies(void)
{
    ...
    for (;;) {
        ...
        fprintf(stderr, "request: %08llx%08llx\n"
                "     op: %u\n"
                "  state: %u\n",
                0LL, (unsigned long long) req->serial, req->op, req->state);
        report_request(req);
    ...
    for (i = 2; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "bridge") && (i + 4 < argc)) {
            ret = cmd_bridge(atol(argv[i + 1]),
                             atol(argv[i + 2]), argv[i + 3], argv[i + 4]);
            i += 4;
            continue;
        }
        ...
        if (!strcmp(argv[i], "rndwrite") && (i + 5 < argc)) {
            ...
            ret =
                cmd_rndwrite(nr_loops, seed, targetlen, chunksize, objectsize);
            i += 5;
            continue;
        }
        ...
        if (!strcmp(argv[i], "rndread") && (i + 5 < argc)) {
            ...
            unsigned long objectsize = atol(argv[i + 5]);
            ret =
                cmd_rndread(nr_loops, seed, targetlen, chunksize, objectsize);
            i += 5;
            continue;
        }
