...
{
    return (struct xheap_header *) ((unsigned long) ptr -
...
//      printf("bytes: %llu\n", bytes);
    ...
    addr =
        (void *) (((unsigned long) mem) + heap->cur +
                  sizeof(struct xheap_header));
...
//                      addr-mem);
    ...
    if (addr
        && xheap_get_chunk_size(addr) !=
        (__get_alloc_bytes(heap, req_bytes) - sizeof(struct xheap_header))) {
        XSEGLOG
            ("allocated chunk size %llu, but it should be %llu (req_bytes %llu)",
             xheap_get_chunk_size(addr), __get_alloc_bytes(heap, req_bytes),
             req_bytes);
        addr = NULL;
    }
    return addr;
...
static inline void __add_in_free_list(struct xheap *heap, xptr * list,
                                      void *ptr)
{
...
int xheap_init(struct xheap *heap, uint64_t size, uint32_t alignment_unit,
               void *mem)
{
