...
void xq_init_map(struct xq *xq,
                 xqindex size,
                 xqindex count, xqindex(*mapfn) (xqindex), void *mem)
{
    xqindex t, *qmem = mem;
    xq->size = __snap(size);
    if (count > xq->size) {
...
xqindex *xq_alloc_map(struct xq * xq,
                      xqindex size, xqindex count, xqindex(*mapfn) (xqindex))
{
    xqindex *mem = xtypes_malloc(size * sizeof(xqindex));
    if (!mem) {
        return mem;
...
void xq_print(struct xq *xq)
{
    ...
    XSEGLOG("xq head: %lu, tail: %lu, size: %lu\n",
            ...
            (unsigned long) xq->tail, (unsigned long) xq->size);
    i = xq->tail + 1;
    ...
    for (;;) {
        ...
        XSEGLOG("%lu %lu\n",
                (unsigned long) i, (unsigned long) xq_element(xq, i));
        i += 1;
    }
}
