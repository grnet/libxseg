...
static inline void set_dummy_key(xhash_t * xhash, xhashidx idx)
{
    xhashidx *kvs = xhash_kvs(xhash);
    kvs[idx] = DUMMY;
}

static inline void set_dummy_val(xhash_t * xhash, xhashidx idx)
{
    xhashidx *vals = xhash_vals(xhash);
    vals[idx] = DUMMY;
}

static bool item_dummy(xhash_t * xhash, xhashidx idx, bool vals)
{
    bool ret;
    xhashidx *pvals;
    xhashidx *kvs = xhash_kvs(xhash);
...
static inline void set_unused_key(xhash_t * xhash, xhashidx idx)
{
    xhashidx *kvs = xhash_kvs(xhash);
    kvs[idx] = UNUSED;
}

static inline void set_unused_val(xhash_t * xhash, xhashidx idx)
{
    xhashidx *vals = xhash_vals(xhash);
    vals[idx] = UNUSED;
}

static inline bool val_unused(xhash_t * xhash, xhashidx idx)
{
    xhashidx *vals = xhash_vals(xhash);
    return vals[idx] == UNUSED;
}

static bool item_unused(xhash_t * xhash, xhashidx idx, bool vals)
{
    xhashidx *kvs = xhash_kvs(xhash);
    if (!vals) {
...
static ssize_t get_alloc_size(xhashidx size_shift, bool vals)
{
    xhashidx nr_items = 1UL << size_shift;
...
xhash_t *xhash_new__(xhashidx size_shift, xhashidx minsize_shift,
                     xhashidx limit, enum xhash_type type, bool vals)
{
    struct xhash *xhash;
...
xhash_t *xhash_resize__(struct xhash * xhash, xhashidx new_size_shift,
                        xhashidx new_limit, bool vals)
{
    return xhash_new__(new_size_shift, xhash->minsize_shift, new_limit,
                       xhash->type, vals);
}

int xhash_delete__(xhash_t * xhash, xhashidx key, bool vals)
{
...
static bool grow_check(xhash_t * xhash)
{
...
static bool shrink_check(xhash_t * xhash)
{
    xhashidx size_shift = xhash->size_shift;
...
void static inline xhash_upd_set(xhash_t * p, xhashidx idx, xhashidx key,
                                 xhashidx val)
{
    xhashidx *kvs = xhash_kvs(p);
    xhashidx *vals = xhash_vals(p);
...
static inline void inc_val(xhash_t * p, xhashidx idx, xhashidx val)
{
    xhashidx *vals = xhash_vals(p);
    vals[idx] += val;
}
...
xhash_t *xhash_resize(xhash_t * xhash, xhashidx new_size_shift,
                      xhashidx new_limit, xhash_t * new)
{
    ...
    for (i = 0; i < xhash_size(xhash); i++) {
        if (item_valid(xhash, i, true)) {
            ...
            xhash_insert__(new, *(xhash_kvs(xhash) + i),
                           *(xhash_vals(xhash) + i));
        }
    }
...
int xhash_lookup__(xhash_t * xhash, xhashidx key, xhashidx * idx_ret,
                   bool vals)
{
 ...
 * Pset functions
 */
pset_t *pset_new(xhashidx minsize_shift)
{
    pset_t *pset;
...
void pset_free(pset_t * pset)
{
    xhash_tfree(&pset->ph_);
    free(pset);
}

void pset_tfree(pset_t * pset)
{
    xhash_tfree(&pset->ph_);
}

void pset_resize(pset_t * pset, xhashidx new_size_shift)
{
    pset_t old;
